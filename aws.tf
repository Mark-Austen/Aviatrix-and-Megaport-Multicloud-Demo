resource "aws_vpc" "aws_vpc_tansit_1" {
...
resource "aws_subnet" "aws_vpc_transit_1_subnet_1" {
  vpc_id                  = aws_vpc.aws_vpc_tansit_1.id
  cidr_block              = var.aws_vpc_transit_1_subnet_1
...
resource "aws_route_table" "aws_vpc_transit_1_route_table_1" {
  vpc_id           = aws_vpc.aws_vpc_tansit_1.id
...
resource "aws_route_table_association" "aws_vpc_transit_1_route_table_association" {
  subnet_id      = aws_subnet.aws_vpc_transit_1_subnet_1.id
  route_table_id = aws_route_table.aws_vpc_transit_1_route_table_1.id
}

resource "aws_internet_gateway" "aws_internet_gateway_vpc_transit_1" {
  vpc_id = aws_vpc.aws_vpc_tansit_1.id
...
resource "aws_route" "aws_vpc_transit_1_default_route" {
  route_table_id         = aws_route_table.aws_vpc_transit_1_route_table_1.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.aws_internet_gateway_vpc_transit_1.id
}

resource "aws_vpn_gateway" "aws_vpn_gateway_vpc_transit_1" {
  vpc_id = aws_vpc.aws_vpc_tansit_1.id
...
resource "aws_vpc" "aws_vpc_spoke_1" {
...
resource "aws_subnet" "aws_vpc_spoke_1_subnet_1" {
  vpc_id                  = aws_vpc.aws_vpc_spoke_1.id
  cidr_block              = var.aws_vpc_spoke_1_subnet_1
  map_public_ip_on_launch = true
...
resource "aws_route_table" "aws_vpc_spoke_1_route_table_1" {
  vpc_id = aws_vpc.aws_vpc_spoke_1.id
...
resource "aws_vpn_gateway_route_propagation" "propagation_1" {
  vpn_gateway_id = aws_vpn_gateway.aws_vpn_gateway_vpc_transit_1.id
  route_table_id = aws_route_table.aws_vpc_transit_1_route_table_1.id
}

resource "aws_route_table_association" "aws_vpc_spoke_1_route_table_association" {
  subnet_id      = aws_subnet.aws_vpc_spoke_1_subnet_1.id
  route_table_id = aws_route_table.aws_vpc_spoke_1_route_table_1.id
}

resource "aws_internet_gateway" "aws_internet_gateway_vpc_spoke_1" {
  vpc_id = aws_vpc.aws_vpc_spoke_1.id
...
resource "aws_route" "aws_vpc_spoke_1_default_route" {
  route_table_id         = aws_route_table.aws_vpc_spoke_1_route_table_1.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.aws_internet_gateway_vpc_spoke_1.id
}
...
resource "aws_dx_gateway_association" "dx_gateway_association" {
  ...
  associated_gateway_id = aws_vpn_gateway.aws_vpn_gateway_vpc_transit_1.id
}
  ...
  user_data = templatefile("${path.module}/templates/gatus.tpl",
    {
      ...
      inter    = "${var.gatus_private_ips["azure"]}"
...
resource "aws_dx_connection_confirmation" "confirmation" {
  connection_id = megaport_vxc.aws_vxc_syd_1.csp_connections[0].connection_id
}
...
resource "aws_dx_private_virtual_interface" "aws_dx_private_virtual_interface_1" {
  ...
  vlan              = megaport_vxc.aws_vxc_syd_1.b_end.vlan
  address_family    = "ipv4"
  bgp_asn           = "65001"
